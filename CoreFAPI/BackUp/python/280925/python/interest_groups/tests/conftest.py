...
env_file = os.getenv("config.yaml", ".env.test")
...
async def auth_client_for_real_sess(client):
    ...
    client.cookies.set("sessionId", "d4fbe514-eadc-46e5-808c-e4b707b8f468")
    return client
...
async def readonly_client(client):
    # Явно устанавливаем cookie и headers
    test_client = client
    test_client.cookies.set("sessionId", "test_session")
    test_client.headers.update({
        "Cookie": "sessionId=test_session",
        "Content-Type": "application/json"
    })
    
    with patch("app.services.user.UserService.get_current_user", new_callable=AsyncMock) as mock_auth:
        mock_auth.return_value = MOCK_USERS["test_session"]
        yield test_client
####################################################
