import uuid
from typing import TYPE_CHECKING

import pytest
from database.models import Comments, Likes, Posts
from dependencies.user import get_user
from sqlalchemy import select
from tests.utils.fake_depends import fake_depends_async, override_dependency
from tests.utils.fake_users import fake_auth_user

if TYPE_CHECKING:
    from core.classes.app import CustomFastApi
    from httpx import AsyncClient
    from sqlalchemy.ext.asyncio import AsyncSession


@pytest.mark.likes_router
@pytest.mark.asyncio
async def test_like_comment_valid(
    client: "AsyncClient",
    app: "CustomFastApi",
    session: "AsyncSession",
) -> None:
    """
    Тестирует успешное создание лайка на комментарии через API.

    Шаги теста:
    1. Создание тестовых данных:
       - Пост с комментарием
    2. Аутентификация пользователя (подмена)
    3. Вызов эндпоинта POST /api/news/comment/{comment_id}/like
    4. Проверка успешного создания лайка

    Проверки:
    - Статус ответа 200 OK
    - Наличие записи в БД
    - Корректность привязки лайка к пользователю и комментарию
    """

    user = fake_auth_user

    # Создадим пост и коммент к нему.
    post = Posts(user_id=uuid.uuid4(), title="test", content="test")
    comment = Comments(user_id=uuid.uuid4(), comment="test")
    post.comments.append(comment)
    session.add(post)
    await session.commit()
    assert post.id
    assert comment.id

    fake_depends_list = [(get_user, fake_depends_async(user))]
    # Поставим лайк комментарию
    async with override_dependency(app, fake_depends_list):
        response = await client.post(f"/api/news/comment/{comment.id}/like")
        assert response.status_code == 200

    # Найдём его в БД
    like_q = await session.execute(
        select(Likes).where(Likes.user_id == user.id, Likes.comment_id == comment.id)
    )
    like = like_q.scalars().one_or_none()
    assert like
