...
@router.post(
    "/",
    response_model=GroupWithMembers,  # Измените на правильный тип ответа
...
async def create_new_group(
    ...
    try:
        # Создаём группу
        db_group = await GroupService.create_group(
            db=db,
            group_data=group,
        ...
        # Явно обновляем связи
        await db.refresh(db_group, ["creator", "members"])

        # Получаем информацию о создателе
        creator = await UserService.get_user_by_id(user_data["user_id"])  # Функция для получения информации о пользователе

        # Получаем участников (пока только создатель группы)
        members = [creator]  # Возможно, позже нужно будет добавить других участников, если это необходимо

        # Преобразуем в Pydantic модель и передаем creator и members
        return GroupWithMembers.from_orm(db_group, creator=creator, members=members)
