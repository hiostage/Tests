...
class GroupWithMembers(Group):
    ...
    is_member: bool = Field(  # Убираем Optional и делаем обязательным булевым значением
        False,  # Значение по умолчанию, если не передано, например, False
        description="Принадлежит ли текущий пользователь к группе"
    )
    
    @classmethod
    def from_orm(cls, group: Group, user_id: int):
        # Определяем, является ли пользователь членом группы
        is_member = any(member.id == user_id for member in group.members)
        return cls(
            creator=group.creator,
            members=group.members,
            is_member=is_member,  # передаем булево значение
        )
