...
def from_orm(cls, group: Group, user_id: int,
    ...
    # Безопасное извлечение creator
    if creator is None:
        if group.creator is not None:
            creator = UserPublic.from_orm(group.creator)
        else:
            creator = UserPublic(
                id=0, username="unknown", email=None, is_active=False
            )  # заглушка — можно заменить на None, если поле допускает

    # Безопасное извлечение members
    if members is None:
        members = []
        for m in group.members or []:
            try:
                members.append(UserPublic.from_orm(m))
            except Exception as e:
                print(f"Ошибка сериализации участника: {e} — {m}")
