...
class Post(Base):
    ...
    created_at = Column(DateTime, default=lambda: datetime.utcnow(), index=True, comment="Дата создания")
    updated_at = Column(DateTime, onupdate=lambda: datetime.utcnow(), comment="Дата обновления")
    
    # Внешние ключи
    author_id = Column(String, ForeignKey("users_cache.id"), index=True, comment="ID автора")
    ...
    last_activity_at = Column(DateTime, onupdate=lambda: datetime.utcnow(), comment="Последняя активность")
    edited_by = Column(String, ForeignKey("users_cache.id"), nullable=True, comment="Кто редактировал")
    ...
    async def is_editable_by(self, db: AsyncSession, user_id: str) -> bool:
