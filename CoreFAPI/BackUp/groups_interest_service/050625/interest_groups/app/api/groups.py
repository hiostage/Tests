...
async def update_existing_group(
    ...
    db_group = await GroupService.get_group(db, group_id=group_id, user_id=user_data["uuid"])
    if not db_group:
...
async def delete_existing_group(
    ...
    try:
        await GroupService.delete_group(db=db, group_id=group_id, user_id=user_data["uuid"])
    except Exception as e:
