...
async def test_rabbitmq_message_delivery():
    ...
    connection = await aio_pika.connect_robust("amqp://user:password@rabbitmq/") # Подключение к брокеру
...
@pytest.mark.asyncio
async def test_auth_client_for_real_sess(auth_client_for_real_sess):
    response = await auth_client_for_real_sess.get("/api/v1/groups/check_auth")
...
@pytest.mark.asyncio
##### Работает только с реальной сессией #####
async def test_get_groups(auth_client_for_real_sess: AsyncClient):
    with patch("app.services.group.GroupService.get_groups", new_callable=AsyncMock) as mock_service:
        ...
        response = await auth_client_for_real_sess.get("/api/v1/groups/")
...
async def test_add_member_to_group(
    ...
    group_with_members = group_with_members_factory(
        ...
        name=mock_group.name,
        creator=mock_creator,
        members=[mock_creator, user_factory],
    )

    with (
        patch("app.api.groups.get_current_user", AsyncMock(return_value={"uuid": mock_creator.id})),
        patch("app.services.user.UserService.sync_user", AsyncMock()),
        ...
        response = await auth_client.post(f"/api/v1/groups/{mock_group.id}/members/{user_factory.id}")
        assert response.status_code == 200

        data = response.json()
        assert data["id"] == mock_group.id
        assert any(m["id"] == user_factory.id for m in data["members"])
...
async def test_remove_member_from_group(
    ...
    updated_group = group_with_members_factory(
        members=[mock_creator]  # Только создатель остался
    )

    with (
        patch("app.services.group.GroupService.get_group", AsyncMock(return_value=group_with_members)),
        patch("app.services.group.GroupService.remove_member", AsyncMock(return_value=updated_group)),
        patch("app.utils.auth.get_current_user", AsyncMock(return_value={"uuid": mock_creator.id}))
    ):
        response = await auth_client.delete(
            f"/api/v1/groups/{group_with_members.id}/members/{user_factory.id}"
        )

        assert response.status_code == 200
        data = response.json()
        assert data["id"] == group_with_members.id
        assert all(m["id"] != user_factory.id for m in data["members"])

@pytest.mark.asyncio
async def test_update_group(auth_client: AsyncClient, mock_creator: User, mock_group: Group, group_with_members_factory):
    # Убедимся, что creator_id совпадает
    mock_group.creator_id = mock_creator.id
    
    updated_group = group_with_members_factory(
        members=[mock_creator]
    )
...
@pytest.mark.asyncio
async def test_delete_group(auth_client: AsyncClient, mock_creator: User, mock_group: Group):
