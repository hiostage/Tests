...
@pytest.mark.asyncio
async def test_delete_subscription_no_auth_user(
    client: "AsyncClient", app: "CustomFastApi"
) -> None:
    """
    Тестирует попытку удаления подписки анонимным (неавторизованным) пользователем.
    Ожидается ошибка 403 Forbidden.

    Шаги:
    1. Использует анонимного пользователя (фикстура fake_anonymous_user).
    2. Отправляет запрос DELETE на отписку с произвольным UUID автора.
