...
func InitRouters(router *http.ServeMux, handlers *ActiveHandlers, middlewares *ActiveMiddlewares) {
	// Auth
	router.Handle("POST /register", middleware.ChainMiddleware()(handlers.HandlerAuth.Register()))

	router.Handle("POST /login", middleware.ChainMiddleware()(handlers.HandlerAuth.Login()))
