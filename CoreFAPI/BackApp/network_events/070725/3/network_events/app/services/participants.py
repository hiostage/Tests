...
class ParticipantService():
    ...
    async def register_user_to_event(
        ...
        if result.scalars().first():
            await self.handle_error(UserAlreadyRegistered, f"Данный пользователь уже зарегистрирован")
    ...
    async def handle_error(self, exception_cls: type[Exception], log_message: str | None = None):
        if log_message:
