...
class GroupService:
    ...
    async def get_groups(
        ...
        """Получение списка групп с пагинацией, фильтрацией по категории и поиском"""
        stmt = select(Group).options(selectinload(Group.creator))

        # Если category не передано, устанавливаем значение по умолчанию
        if category is None:
            category = 'other'  # Или любое другое значение по умолчанию
