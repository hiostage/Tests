...
class GroupService:
    ...
    async def add_member_to_group(db: AsyncSession, group_id: int, user_id: int, role: str = "member"):
        ...
        group_result = await db.execute(update_group)
        group = group_result.scalar_one()
        group.member_count += 1
        db.add(group)  # Добавляем измененную группу в сессию
        await db.commit()

        # Получаем информацию о группе и её участниках
        group_stmt = select(Group).filter_by(id=group_id)
        group_result = await db.execute(group_stmt)
        group = group_result.scalar_one()

        # Создаём объект GroupWithMembers
        creator_stmt = select(User).filter_by(id=group.creator_id)
        creator = await db.execute(creator_stmt)
        creator = creator.scalar_one()
        ...
        return GroupWithMembers.from_orm(
            ...
            is_member=True  # Указываем, что текущий пользователь теперь участник группы
        )
