...
class PostService():
    ...
    async def get_post(self, post_id: int) -> Optional[PostWithAuthor]:
        ...
        if not post:
            ...
            self.handle_error(PostNotFound)
    ...
    @handler_decorator
    async def create_post(self, post_data: PostCreate, author_id: str) -> PostWithAuthor:
        ...
        author = await UserService.get_local_user(self.db, author_id)
        if not author:
            self.handle_error(AuthorNotFound, f"Author {author_id} not found")
        # Создаем новый пост
        db_post = Post(
            **post_data.dict(),
            author_id=author_id,
    ...
    async def update_post(self, post_id: int, post_data: PostUpdate) -> PostWithAuthor:
        """Обновление поста"""
        db_post = await self.interactions_with_post(post_id)
