...
async def create_new_group(
    ...
    creator = await UserService.get_user_by_id(db, user_data["uuid"])
        
    if not creator:
        raise HTTPException(status_code=404, detail="Creator not found")
        
    # Создаём группу
    group_services = GroupService(db)
    group = await group_services.create_group(group_data=group, creator_id=user_data["uuid"], request=request)

    # Формируем объект GroupWithMembers с добавленными участниками и создателем
    group_with_members = GroupWithMembers.from_group(group, user_id=user_data["uuid"], creator=creator, members=[])
...
async def check_auth(user_data: dict = Depends(get_current_user)):
    ...
    return {"status": "authenticated", "user_id": user_data["uuid"]}
...
async def read_groups(
    ...
    try:
        data = await group_services.get_groups(skip=skip, limit=limit, category=category, search=search, user_id=user_id)
...
async def read_group(
    ...
    group_services = GroupService(db)
    db_group = await group_services.get_group(group_id=group_id, user_id=user_id)
    if not db_group:
...
async def update_existing_group(
    ...
    group_services = GroupService(db)
    db_group = await group_services.get_group(group_id=group_id, user_id=user_data["uuid"])
    if not db_group:
        raise HTTPException(status_code=404, detail="Group not found")
    
    if db_group.creator_id != user_data["uuid"]:
        raise HTTPException(status_code=403, detail="Only group creator can update group")
    
    
    return await group_services.update_group(group_id=group_id, group_data=group, user_id=user_data["uuid"])
...
async def delete_existing_group(
    ...
    try:
        await group_services.delete_group(group_id=group_id, user_id=user_data["uuid"])
    except Exception as e:
...
async def add_member(
    ...
    group_with_members = await group_services.add_member_to_group(group_id=group_id, user_id=user_id)
...
async def remove_member(
    ...
    if db_group.creator_id != user_data["uuid"] and user_data["uuid"] != user_id:
