...
async def create_new_group(
    request: Request,  # 1. Сначала Request
    group: GroupCreate,  # 2. Затем тело запроса
    db: AsyncSession = Depends(get_db),  # 3. Зависимости
    user_data: dict = Depends(get_current_user)  # 4. Ещё зависимости
):
    """Create a new group"""
