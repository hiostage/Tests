...
class UserService:
    ...
    async def get_user_from_auth_service(user_id: int) -> Optional[UserInDB]:
        """
        Получение данных пользователя из внешнего сервиса (fallback)
        """
        try:
            async with httpx.AsyncClient(timeout=settings.REQUEST_TIMEOUT) as client:
                response = await client.get(
                    f"{settings.AUTH_PROXY_URL}/users/{user_id}",
                    headers={"Authorization": f"Bearer {settings.AUTH_PROXY_URL}"}
                )
                if response.status_code == 200:
    ...
    async def get_or_fetch_user(
        ...
        if user_id:
            ...
            external_user = await UserService.get_user_from_auth_service(user_id)
            if external_user:
