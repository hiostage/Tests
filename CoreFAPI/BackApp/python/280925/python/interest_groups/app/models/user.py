...
class User(Base):
    ...
    async def update_from_external(self, db: AsyncSession, external_data: UserFull):
        """Обновление данных из внешнего сервиса (асинхронно)"""
        stmt = (
            update(User)
            .where(User.id == self.id)
            .values(
                username=external_data.username or self.username,
                first_name=external_data.first_name or self.first_name,
                last_name=external_data.last_name or self.last_name,
                email=external_data.email or self.email,
