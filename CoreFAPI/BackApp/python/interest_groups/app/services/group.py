...
class GroupService:
    @staticmethod
    async def add_member_to_group(db: AsyncSession, group_id: int, user_id: str, role: str = "member"):
    ...
    @staticmethod
    @handle_db_errors
    async def get_group(db: AsyncSession, group_id: int, user_id: str):
    ...
    @staticmethod
    @handle_db_errors
    async def is_member(db: AsyncSession, group_id: int, user_id: str) -> bool:
    ...
    async def create_group(db: AsyncSession, group_data: GroupCreate, creator_id: int, request: Request) -> GroupWithMembers:
        ...
        if not creator:
            logger.error(f"Creator not found for group creation. creator_id: {creator_id}")
        ...
        if existing_group.scalars().first():
            logger.error(f"Group with name {group_data.name} already exists")
    ...
    @staticmethod
    @handle_db_errors
    async def update_group(db: AsyncSession, group_id: int, group_data: GroupUpdate, user_id: str) -> GroupWithMembers:
        """Обновление информации о группе"""
        stmt = select(Group).filter(Group.id == group_id)
        result = await db.execute(stmt)
        db_group = result.scalars().first()
        if not db_group:
            logger.error(f"Group with ID {group_id} not found for update")
            raise HTTPException(status_code=404, detail="Group not found")
    ...
    @staticmethod
    @handle_db_errors
    async def delete_group(db: AsyncSession, group_id: int, user_id: str) -> GroupWithMembers:
        """Удаление группы"""
        stmt = select(Group).filter(Group.id == group_id)
        result = await db.execute(stmt)
        db_group = result.scalars().first()
        if not db_group:
            logger.error(f"Group with ID {group_id} not found for deletion")
            raise HTTPException(status_code=404, detail="Group not found")
    ...
    @staticmethod
    @handle_db_errors
    async def remove_member(db: AsyncSession, group_id: int, user_id: str) -> dict:
        ...
        if not db_group:
            logger.error(f"Group with ID {group_id} not found for member removal")
            raise HTTPException(status_code=404, detail="Group not found")
        ...
        if not db_user:
            logger.error(f"User with ID {user_id} not found for removal from group {group_id}")
