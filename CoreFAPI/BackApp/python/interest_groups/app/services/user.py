...
class UserService:
    ...
    async def sync_user(db: AsyncSession, user_data: dict) -> User:
        result = await db.execute(select(User).where(User.id == user_data["uuid"]))
        ...
        if not user:
            stmt = insert(User).values(
                ...
                username=user_data["userName"],
                email=user_data["email"],
                is_active=user_data.get("is_active", True),  # если есть в user_data
            )
            try:
                await db.execute(stmt)
                await db.commit()
                logger.info(f"User {user_data['uuid']} synchronized successfully")
            except IntegrityError:
                await db.rollback()
                logger.warning(f"User {user_data['uuid']} already exists, skipping insertion")
            # Получим вновь созданного пользователя
            result = await db.execute(select(User).where(User.id == user_data["uuid"]))
            user = result.scalar_one()
        else:
            await user.update_from_external(db, user_data)
            logger.info(f"User {user_data['uuid']} updated successfully")
