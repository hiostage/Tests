...
async def create_new_post(
    ...
    try:
        return await PostService.create_post(db=db, post_data=post, author_id=current_user.id)
...
async def read_posts(
    ...
    return await PostService.get_posts(db=db, skip=skip, limit=limit)
...
async def read_posts_by_group(
    ...
    return await PostService.get_posts_by_group(db, group_id=group_id, skip=skip, limit=limit)
...
async def update_post(
    ...
    post = await PostService.get_post_by_id(db, post_id)  # Получаем пост по ID
    if not post:
