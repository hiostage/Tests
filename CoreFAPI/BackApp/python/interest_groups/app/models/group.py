...
group_members = Table(
    ...
    Column('user_id', String, ForeignKey('users_cache.id'), primary_key=True, comment="ID пользователя"),
    Column('joined_at', DateTime, default=lambda: datetime.utcnow(), comment="Дата вступления"),
...
class Group(Base):
    ...
    creator_id = Column(String, ForeignKey("users_cache.id"), index=True, comment="ID создателя")
    ...
    async def is_member(self, db: AsyncSession, user_id: str) -> bool:
